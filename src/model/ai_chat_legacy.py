...
SYSTEM_INSTRUCTION = """
이 실행기는 이전 버전 호환용입니다. 운영 여행 대화는 서버 상태 머신 실행기로 전달됩니다.
직접 호출되더라도 장소 검색, 경로 조회, 장소 추천 또는 일정 본문을 생성하지 마세요.
최근 대화에서 사용자가 명시한 여행 조건만 짧게 확인하고, 필요한 질문은 한 번에 하나만 하세요.
장소명, 내부 도구명, 검증 과정, 오류 코드나 서버 구현 용어를 출력하지 마세요.
기존 조건을 임의로 덮어쓰거나 추측하지 말고 자연스러운 한국어 한두 문장으로만 답하세요.
""".strip()
...
class AiChat:
    def send(self, prompt, history_raw="[]", user_id="", thread_id=""):
        ...
        try:
            ...
            return status, dict(message=self._error_message(error), _fallback_reason="legacy_provider_error")
        except Exception:
            return 502, dict(
                message="AI 응답을 가져오지 못했습니다. 잠시 후 다시 시도해주세요.",
                _fallback_reason="legacy_unexpected_error",
            )

        if not reply:
            return 502, dict(message="AI 응답이 비어 있습니다. 다시 질문해주세요.", _fallback_reason="legacy_empty_response")
        ...
        if result.get("fallback_reason"):
            payload["_fallback_reason"] = result.get("fallback_reason")
    ...
    def _call_gemini(self, prompt, history):
        ...
        for _ in range(MAX_TOOL_LOOPS):
            ...
            last_validation_failed = bool(unknown_mentions)
            if unknown_mentions:
            ...
            return {
                ...
                "fallback_reason": "legacy_unverified_reply" if unknown_mentions else "",
            }
        ...
        return {
            ...
            "tool_logs": tool_logs,
            "itinerary_proposal": self._extract_itinerary_proposal(reply),
            "fallback_reason": "legacy_unverified_reply" if last_validation_failed else "legacy_tool_budget_exhausted",
        }
    ...
    def _log_tool_event(self, log):
        ...
        payload = {
            ...
            "has_response": bool(log.get("functionResponse")) if isinstance(log, dict) else False,
        }
        try:
