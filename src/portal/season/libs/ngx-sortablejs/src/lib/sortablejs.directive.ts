...
const getIndexesFromEvent = (event: SortableEvent) => {
    if (event.hasOwnProperty('newDraggableIndex') && event.hasOwnProperty('oldDraggableIndex')) {
        return {
            new: event.newDraggableIndex ?? event.newIndex ?? 0,
            old: event.oldDraggableIndex ?? event.oldIndex ?? 0,
        };
    } else {
        return {
            new: event.newIndex ?? 0,
            old: event.oldIndex ?? 0,
        };
    }
};
...
export class SortablejsDirective implements OnInit, OnChanges, OnDestroy {
    ...
    @Input()
    sortablejsCloneFunction: ((item: any) => any) | null = null;
        ...
        return {
            onAdd: (event: SortableEvent) => {
                this.service.transfer = (items: any[]) => {
                    this.getBindings().injectIntoEvery(event.newIndex ?? 0, items);
            ...
            onRemove: (event: SortableEvent) => {
                ...
                if (bindings.provided) {
                    const transfer = this.service.transfer;
                    if (!transfer) return;
                    if (this.isCloning) {
                        transfer(bindings.getFromEvery(event.oldIndex ?? 0).map(item => this.clone(item)));
                        this.renderer.removeChild(event.item.parentNode, event.item);
                        this.renderer.insertBefore(event.clone.parentNode, event.item, event.clone);
                        this.renderer.removeChild(event.clone.parentNode, event.clone);
                    } else {
                        transfer(bindings.extractFromEvery(event.oldIndex ?? 0));
                    }
